...
{
	//__android_log_print(ANDROID_LOG_INFO, "libSDL", "nativeMouse called");
	if(pointerId < 0)
		pointerId = 0;
	...
	{
		SDL_keysym keysym;
	        __android_log_print(ANDROID_LOG_INFO, "libSDL", "Sending SDL_KEY event");
		if( action != MOUSE_MOVE )
	...
	{
	        //__android_log_print(ANDROID_LOG_INFO, "libSDL", "Sending Mouse event");
#if SDL_VERSION_ATLEAST(1,3,0)
...
{
	__android_log_print(ANDROID_LOG_INFO, "libSDL", "Inside nativeKey: Key:%d Action:%d", key, action);
	if( isTrackballUsed )
