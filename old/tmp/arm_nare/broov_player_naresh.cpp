	...
	if (vp->pFrameRGB) {
		...
		if (vp->display_rgb_image == NULL) {
			...
			vp->display_rgb_image = SDL_CreateRGBSurfaceFrom(vp->pFrameRGB->data[0], 
					...
					0, 0, 0, 0);
#ifdef RGB_PLAYER_32
			vp->display_rgb_text = SDL_CreateTextureFromSurface(SDL_PIXELFORMAT_RGB32, vp->display_rgb_image);
#else
...
#ifdef RGB_PLAYER_32
    vp->numBytes = avpicture_get_size(PIX_FMT_RGB32, is->video_st->codec->width, is->video_st->codec->height); 
#else
	vp->numBytes = avpicture_get_size(PIX_FMT_RGB24, is->video_st->codec->width, is->video_st->codec->height); 
#endif	
...
static int rgb_queue_picture(VideoState *is, AVFrame *pFrame, double pts, int64_t pos) 
{
	...
	if (vp->pFrameRGB) {
        ...
        __android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Before ARM YUV to RGB");
		//img_convert((AVPicture*) vp->pFrameRGB, dst_pix_fmt,
		//		(AVPicture *)pFrame, is->video_st->codec->pix_fmt, 
		//		is->video_st->codec->width, is->video_st->codec->height);
		mb_YUV420_to_RGB32(int size, int lumaStep, char *yuv[], char *dest);
		mb_YUV420_to_RGB32( ( (is->video_st->codec->width << 16) | (is->video_st->codec->height)),
                      pFrame->linesize[0], (char**)pFrame->data, (char*)vp->buffer);

        __android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "After ARM YUV to RGB");					  
...
static int video2_thread(void *arg) 
{
	...
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Video2 Thread");
	for(;;) {

		while (is->paused && !is->video2q.abort_request) {
			//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Video Paused");
			SDL_Delay(10);
		}

		//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Video2Thread Frame Get");
		ret = vpacket_queue_get(&is->video2q, &frame, &pts, &pos, 1);
		//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Video2Thread Frame Received: frame:%p flush_frame:%p Pts:%lf Pos:%ld FList:%p", frame, &flush_frame, pts, pos, frameList);
		if (ret < 0) goto the_end;

		if (!ret)
			continue;
		if (pts == flush_pts && pos == flush_pos) {
			__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Received flush frame");
			continue;
...
static int video_thread(void *arg) 
{
	...
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "VideoThread ");
	for(;;) {
		AVPacket pkt;
		...
		if (!ret)
			continue;
		//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Received video frame");

		pts = pts_int*av_q2d(is->video_st->time_base);

		//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "VideoThread Before Frame Put");
		vpacket_queue_put(&is->video2q, frame,  &pts, &pkt.pos);
		//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "VideoThread After Frame Put");
...
static double calculate_file_fps(VideoState *is)
{
	...
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "rfps : %lf", rfps);
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "rfps_base : %lf", rfps_base);	
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "secondsPerFrame data : %lf", is->video_st->codec->time_base );
	...
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "fps : %lf", av_q2d(is->video_st->avg_frame_rate));
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "tbr : %lf", av_q2d(is->video_st->r_frame_rate));
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "tbn : %lf", 1/av_q2d(is->video_st->time_base));
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "tbc : %lf", 1/av_q2d(is->video_st->codec->time_base));	
	if (is->video_st->codec->time_base.den != rfps*is->video_st->codec->ticks_per_frame || is->video_st->codec->time_base.num != rfps_base) {
		...
		// __android_log_print(ANDROID_LOG_INFO,"Seems stream codec frame rate differs from container frame rate:  %d/%d",
		//			  rfps, rfps_base);
		//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "codec ratio : %lf", ((float)is->video_st->codec->time_base.den / is->video_st->codec->time_base.num));
		//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "codec timebase.den : %lf", is->video_st->codec->time_base.den);
		//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer Naresh", "codec timebase.num : %lf", is->video_st->codec->time_base.num);	
...
static int skip_now(VideoState *is)
{
	...
	//calculate_frames_per_second_internal_old();
	
	//file_fps	=	calculate_file_fps(is);
	//calculate_video_lag();
	//static int skipafternumberofframes=2;
	//static int skipafternumberofframessmall=3;
	//static bool alternateskip = true;
	//static int skipnumberoftimes1 = 2;
	//static int skipnumberoftimes2 = 3;
	//static int skipnumberoftimeslarge1 = 5;
	...
	//if(audio_pkt_dts < video_pkt_dts + 10){
	//audio_pkt_dts= 60;
	//video_pkt_dts= 30;
	//}else if(audio_pkt_dts > video_pkt_dts + 10){
	//audio_pkt_dts= 30;
	//video_pkt_dts= 15;
	//}
...
static int video_thread(void *arg) 
{
	...
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Inside video_thread vlref_clock: %lf", vlref_clock);
	for(;;) {
		AVPacket pkt;
		...
		double systemMasterClock = current_time();
...
/* if the queue are full, no need to read more */
	...
	if ((is->audioq.size + is->videoq.size)> (1024*1024))
	{
		...
		//if (is->videoq.nb_packets>10 && is->videoStream>=0)
		...
		//	continue;
		//}
		//if (is->audioq.nb_packets>15 && is->audioStream>=0)
