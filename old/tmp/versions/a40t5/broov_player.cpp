...
//These two defines are only test codes, not to be used
//#define BROOV_PLAYER_OPTIMIZED
//#define BROOV_PLAYER_ASSEMBLY
...
{
	VideoPicture *vp;
	PixelFormat dst_pix_fmt = PIX_FMT_RGB565;
...
static int output_picture2(VideoState *is, AVFrame *src_frame, double pts1, int64_t pos)
{
	...
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Before RQP");
...
static int get_video_frame(VideoState *is, AVFrame *frame, int64_t *pts, AVPacket *pkt)
{
	int len1, got_picture, i;
        ...
        // Do not block the code if no data is available
	...
	if (ret < 0)
        ...
        if (ret == 0) return 0; //no packet is received now
...
static int video_module(void *arg) 
{
...

	{
		...
		if (!ret) return 2;
	        ...
	        //__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "VFP");
#ifdef BROOV_VIDEO_SKIP
...
#ifdef BROOV_PLAYER_OPTIMIZED
uint8_t         *buffer;
AVFrame         *pFrameRGB;
struct SwsContext *img_convert_ctx;
SDL_TextureID text; 
int broov_init_optimized(VideoState *is)
{

        if (!is->video_st) return 0;
	...
	numBytes=avpicture_get_size(PIX_FMT_RGB565, is->video_st->codec->width,
	...
	avpicture_fill((AVPicture *)pFrameRGB, buffer, PIX_FMT_RGB565,
        ...
        img_convert_ctx = sws_getCachedContext(img_convert_ctx,
				...
				pCodecCtx->height, dst_pix_fmt, sws_flags, 
				NULL, NULL, NULL);
...
void broov_init_optimized_cleanup()
{
...
static int display_rgb_picture_type2(AVFrame *pFrame, int width, int height)
{
	...
	calculate_frames_per_second();
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Processed Display FPS");


	if (!text) {

	    __android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "BCT2");
	    text = SDL_CreateTexture(SDL_PIXELFORMAT_RGB565, SDL_TEXTUREACCESS_STATIC, width, height);
	    __android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "ACT2");
        } else {

	    //__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "BUT");
	    SDL_UpdateTexture(text, NULL, pFrame->data[0], pFrame->linesize[0]);
	    //__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "AUT");
        }

        //__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "BRC");
        SDL_RenderCopy(text, NULL, NULL);
        //__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "ARC");

	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "BRP");
	SDL_RenderPresent();
	//__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "ARP");
...

	{
		...
		if (packet->stream_index == is->videoStream) {
#ifdef BROOV_PLAYER_OPTIMIZED
			{
				...
				if (frameFinished) {
#ifdef BROOV_PLAYER_ASSEMBLY
                                  yuv420_2_rgb565(pFrameRGB->data[0], 
				                  g_video_frame->data[0], 
						  g_video_frame->data[1], 
						  g_video_frame->data[2],
						  is->video_st->codec->width, 
						  is->video_st->codec->height, 
						  g_video_frame->linesize[0],  // Y span
						  g_video_frame->linesize[1],  // UV span Width / 2 
						  (is->video_st->codec->width<<1),  //Dest width* bpp (width *2bytes)
						  yuv2rgb565_table, 
						  dither++);

#else
					sws_scale(img_convert_ctx, g_video_frame->data, g_video_frame->linesize,
							0, is->video_st->codec->height, pFrameRGB->data, pFrameRGB->linesize);

#endif

					display_rgb_picture_type2
						(pFrameRGB, is->video_st->codec->width, is->video_st->codec->height); 
				}
			}
...
static int decode_thread(void *arg)
{
    ...
    while (1) {
	   ...
	   if (is->video_st && !video_module_quit) {
               //Process a video frame from queue
         
               vid_ret_value = video_module(is);

               if (vid_ret_value == 3) video_module_quit = 1;
               if (vid_ret_value == 1) { SDL_Delay(10); continue; }
           } else {
               if ((ret_value>=1 && ret_value <=4) ||(ret_value==6)) {
	...
	__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Flush Pkt Init Successful");
	   ...
	   //if ((ui_scheduler%64) == 0)
		   ...
		   do {
			   ...
			   if (!ret_value) {
				   //no data available in UI event queue
				   break;
			   }
		   } while(1);
